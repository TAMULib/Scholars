...
A theme can be deleted by passing the _theme _id_ as a path parameter. The _active_ flag has to be set to false prior to delete. This will return a *NO_CONTENT (204)* http status code as shown in the *Sample Response*.
...
A user, with an admin role, can find a theme by providing _theme _id_ as a path parameter. This returns a json response displaying the  *HttpStatus.OK(200)*  message along with the theme object as shown in the *Sample Response*.
