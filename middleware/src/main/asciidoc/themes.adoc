...
==== Delete
A theme can be deleted by passing the _theme_ _id_ as a path parameter. The _active_ flag has to be set to false prior to delete. This will return a *NO_CONTENT (204)* http status code as shown in the *Sample Response*.
...
==== Find by ID
A user, with an admin role, can find a theme by providing theme _id_ as a path parameter. This returns a json response displaying the  *HttpStatus.OK(200)*  message along with the theme object as shown in the *Sample Response*.
...
==== Update
This helps an admin user to update a specifc theme based on the _theme id_ provided in the path parameter. This utilizes the hhtp put protocol.

===== Path Parameters
include::{snippets}/themes/update/path-parameters.adoc[]

===== Request Parameters
include::{snippets}/themes/update/request-parameters.adoc[]
