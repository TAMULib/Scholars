...
A discovery view is created by adding a fields like _name_, _collection_, _resultView_, _facets_, _filters_ etc along with other variables as shown in the *Request Parameters*.
...
A discovery view can be deleted by passing the _discoveryView _id_ as a path parameter. This will return a *NO_CONTENT (204)* http status code as shown in the *Sample Response*.
...
A user can find a discovery view by providing _discoveryView _id_ as a path parameter. This returns a json response displaying the  *HttpStatus.OK(200)*  message along with the theme object as shown in the *Sample Response*.
...
This helps an admin user to update a specifc discovery view based on the _discoveryView id_ provided in the path parameter. This utilizes the hhtp put protocol.

===== Path Parameters
include::{snippets}/discoveryViews/update/path-parameters.adoc[]

===== Request Parameters
include::{snippets}/discoveryViews/update/request-parameters.adoc[]
