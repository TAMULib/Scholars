...
A directory view is created by adding a fields like _name_, _collection_, _index_ etc along with other variables as shown in the *Request Parameters*.
...
A directory view can be deleted by passing the _directoryView _id_ as a path parameter. This will return a *NO_CONTENT (204)* http status code as shown in the *Sample Response*.
...
A user can find a directory view by providing _directoryView _id_ as a path parameter. This returns a json response displaying the  *HttpStatus.OK(200)*  message along with the directory view object as shown in the *Sample Response*.
...
===== Request Parameters
include::{snippets}/directoryViews/update/request-parameters.adoc[]
